...
Mat getDepthImage(Mat dispImg)
{	
	Mat depImg(dispImg.rows, dispImg.cols, CV_32FC1);
...
void stereo_sgbm()
{
	...
	Mat depthImg = getDepthImage(disparity_im);
...
//	baseLine = 0.24;
	...
	cx_l = cx_r = 130.0 * scale_;
	cy_l = cy_r = 204.5 * scale_;
