...
__emulated_shfl( const int scalarValue, const uint32_t source_lane )
{
...
__inline__ __device__ int
shfl_32( int scalarValue, const int lane )
{
#if FERMI
...
__inline__ __device__ int
shfl_up_32( int scalarValue, const int n )
{
#if FERMI
...
__inline__ __device__ int
shfl_down_32( int scalarValue, const int n )
{
#if FERMI
    int lane = threadIdx.x % WARP_SIZE;
    lane += n;
...
__inline__ __device__ int
shfl_xor_32( int scalarValue, const int n )
{
#if FERMI
    int lane = threadIdx.x % WARP_SIZE;
    lane     = lane ^ n;
...
st_gbl_wt( const __restrict__ uint32_t* addr, const uint32_t value )
{
    asm( "st.global.wt.u32 [%0], %1;" ::"l"( addr ), "r"( value ) );
}

__device__ __forceinline__ void
st_gbl_cs( const __restrict__ uint32_t* addr, const uint32_t value )
{
...
__device__ __forceinline__ void
uint32_to_uchars( const uint32_t s, int* u1, int* u2, int* u3, int* u4 )
{
    //*u1 = s & 0xff;
...
__device__ __forceinline__ uint32_t
uchars_to_uint32( int u1, int u2, int u3, int u4 )
{
...
__device__ __forceinline__ uint32_t
uchar_to_uint32( int u1 )
{
    return __byte_perm( u1, u1, 0x0 );
}

__device__ __forceinline__ unsigned int
vcmpgeu4( unsigned int a, unsigned int b )
{
    unsigned int r, c;
    c = a - b;
...
__device__ __forceinline__ unsigned int
vminu4( unsigned int a, unsigned int b )
{
    unsigned int r, s;
...
__device__ __forceinline__ void
print_uchars( const char* str, const uint32_t s )
{
    int u1, u2, u3, u4;
...
__inline__ __device__ int
warpReduceMinIndex2( int* val, int idx )
{
...
__inline__ __device__ int
warpReduceMinIndex( int val, int idx )
{
    for ( int d = 1; d < WARP_SIZE; d *= 2 )
    {
        int tmp     = shfl_xor_32( val, d );
...
__inline__ __device__ int
blockReduceMinIndex( int val, int idx )
{
